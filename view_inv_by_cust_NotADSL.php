<title>CustInvNotADSL</title><link rel="stylesheet" href="mystyle.css">
<?php
...
<b><!--<font size = "2" type="arial">-->

<?php 
$SQLstring = "select * from invoice where CustNo = '$CustInt' and summary NOT LIKE '%adsl%' order by InvNo desc";
if (@$un == 'N')
echo "Error - please try view_inv_by_custUNRECONCILED.php";
$un = 'Y';
echo "<br><br>Your Invoices History view_inv_by_cust_NotADSL.php";
...
$TrRDday = $TrRDdate_array[2];
    ...
    $resultSS->close();
			...
			echo "<font color = green>Inv{$row['InvNo']}<br>Paid on {$TrRD}</th>\n";
			$TrRD ="";//reset
			$TrR ="";//reset
...
echo "&nbsp;</th>";  // No Proof received
			...
			if ($resultT = mysqli_query($DBConnect, $trsql)) {
				while ($rowT = mysqli_fetch_assoc($resultT)) {
				//echo "<th>";
				echo "<font color= green>Tr".$rowT['TransNo'];
				echo " (r".$rowT['AmtPaid'].")";
				}
			}	  
...
echo "(All invoices NOT ADSL total to: R ".$Invsummm;
echo ")<br />";

...
<font color=blue> or click here for more details with proofs: <a href = view_inv_by_custADV.php>view_inv_by_custADV.php</a></font>
 ...
 &nbsp;&nbsp;&nbsp;&nbsp;</font> </b><font color=#F5F5DC>view_inv_by_cust.php &nbsp;&nbsp;&nbsp;<br>
...
{
echo "ExpNo".$rowE['ExpNo'].":<br>".$rowE['ExpDesc']." ".$rowE['SerialNo']."<br>";
}
...
echo "<th>";
			...
			$IInv = $row['InvNo'];
			$IIII = $row['InvNo'];
$SS = "select TransNo, TransDate from transaction where InvNoA = '$IInv' or  InvNoB = '$IInv'  or  InvNoC = '$IInv'  or  InvNoD = '$IInv'  or  InvNoE = '$IInv'  or  InvNoF = '$IInv'  or  InvNoG = '$IInv'  or  InvNoH = '$IInv' ";
//echo $SS;
...
$TrRDday = $TrRDdate_array[2];
    ...
    $resultSS->close();
			...
			//if ( $TrR == '')
			echo "Inv{$row['InvNo']}</th>\n";
			//else
			//echo "{$row['InvNo']}<br>Paid on {$TrRD}</th>\n";
			$TrRD ="";//reset
			$TrR ="";//reset
...
//			echo "<th>R{$row[29]}</th>"; ///TOTAL AMOUNT TotAmt
			echo "<th>R{$row['TotAmt']}</th>"; ///TOTAL AMOUNT TotAmt
						...
						$IIII = $row['InvNo'];
		$SQLP = "select * from aproof where InvNoA = '$IIII' or  InvNoB = '$IIII'  or  InvNoC = '$IIII'  or  InvNoD = '$IIII'  or  InvNoE = '$IIII'  or  InvNoF = '$IIII'  or  InvNoG = '$IIII' or  InvNoH = '$IIII'  ";
/*echo "<th>";  // No Proof received

if ($resultP = mysqli_query($DBConnect, $SQLP)) {
	  while (@$rowP = mysqli_fetch_assoc($resultP)) {
			echo "{$rowP['ProofNo']}</font>";  //ProofNo
	}
    $resultP->close();
}		
...
echo "&nbsp;</th>";  // No Proof received
			...
			*/
			...
			if ($resultT = mysqli_query($DBConnect, $trsql)) {
				while ($rowT = mysqli_fetch_assoc($resultT)) {
				//echo "<th>";
				echo "<font color= green>TransNo".$rowT['TransNo'];
				echo " (R".$rowT['AmtPaid'].")";
				}
			}	  

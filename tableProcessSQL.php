...
$DBConnect = mysqli_connect("localhost", "root", "Itsmeagain007#", "kc");
        ...
        header('Content-Disposition: attachment;filename="backup_'.$table.date('Y-m-d_h_i_s') . '.sql"');
...
//echo $table;
            $strSQL = "DESCRIBE ".$table;
           if (!$res_create = $DBConnect->query($strSQL))
               throw new Exception("MySQL Error: " . $DBConnect->error . 'SQL: '.$strSQL);
          while( $row_create = $res_create->fetch_assoc())
		  {
          print($row_create['Type']);
            print(" ".$row_create['Field'].",\n");
 //		   echo "<br>";
		   }
//		   echo "<br>";
//		   echo "<br>";
/*
$res = mysqli_query('DESCRIBE products');
  ...
  //          $res_create->free();
            ...
            while ($values = $res_select->fetch_assoc()) {
                ...
                foreach ($fields_info as $field) {
                    ...
                    $strValues .= '"'.preg_replace('/[^(\x20-\x7F)\x0A]*/','',$values[$field->name].'"'); //preg_replace not deprecated

...
<a href = 'editDomains.php' target='_blank'>editDomains</a></br>

<b><br><font size = "4" type="arial">View Domains</b></font>&nbsp;&nbsp;&nbsp;&nbsp;viewExp.php
</br>
<?php
$queryC = "select * from domains ORDER BY name";
echo $queryC;
...
echo "<tr>";
echo "<th>".$row["name"]."</th>";
echo "<th><a href = '{$row["name"]}?='{$row["name"]}'></a></th>";
echo "<th><a href = 'http://www.{$row["name"]}/1.php' target= _blank>TEST_1.php</a> ";
//echo "<a href='mailto:domains@cybersmart.co.za?&subject=request%20{$row["name"]}=my PHP on my website is not wokring on http://www.{$row["name"]}/1.php' target="_top">Send mail!</a></th>";
//<a href="mailto:someone@example.com?subject=Summer&body=You%20are%20invited%20to%20a%20big%20summer%20party!" target="_top">Send mail!</a>echo "<a href='mailto:domains@cybersmart.co.za?&subject=request%20{$row["name"]}=my PHP on my website is not wokring on http://www.{$row["name"]}/1.php' target="_top">Send mail!</a></th>";


echo "<a href='mailto:domains@cybersmart.co.za?subject=request%20{$row["name"]}&body=my%20PHP%20on%20my%20website%20is%20not%20working%20on%20www.{$row["name"]}/1.php' target='_top'>Send mail</a></th>";
echo "<th><a href = 'http://www.{$row["name"]}' target= _blank>TEST_www</a> ";
echo " <a href = 'http://cpanel.{$row["name"]}' target= _blank>CPanel</a> ";
echo " <a href = 'http://cpanel.{$row["name"]}/cpsess1041249301/frontend/x3/stats/resourceusagedetails.html?period=1d' target= _blank>Resources</a></th>";
echo "<th>".$row["status"]."";
echo "<th>".$row["info"]."";
echo "<th>".$row["CustNo"]."";
echo "<th>".$row["Extra"]."";
echo "<th>".$row["Confid"]."";
echo "<th>".$row["PayNotes"]."";
$row_cnt = mysqli_num_rows($resultC);
echo "</tr>\n";
//http://cpanel.legalladies.co.za/cpsess1041249301/frontend/x3/stats/resourceusagedetails.html?period=1d
}
...
$SQLstring = "select * from expenses  where Category = 'domain' order by ExpNo  desc";
...
<?php
$SQLstring = "select * from expensesH  where Category = 'domain' order by ExpNo  desc";
echo $SQLstring."<br><br>"; //the whole content of the table is now require_onced in a PHP array with the name $QueryResult.

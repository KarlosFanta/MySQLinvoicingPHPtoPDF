...
@$IT= (($Q1*$ex1+$Q2*$ex2+$Q3*$ex3+
			...
			$Q7*$ex7 + $Q8*$ex8)*1.15);
			$IT = number_format ($IT, 2, ".", ""); //A non-numeric value encountered 
			//echo " R".$IT;
//echo "<br>";echo "<br>";
...
if ($CustNo == 10) //for HKittmann who never pays VAT
	echo "<form  action='PDF/tcpdf/examples/PDFharald.php' method='post' id = 'form1'>";
else
	echo "<form  action='PDF/tcpdf/examples/PDF.php' method='post' id = 'form1'>";

echo "Cn<input type='text' name='CustNo' id='CustNo' size = '2' value='$CustNo' />";
...
<a href = "assignCustExpToInvGET.php?CustNo=<?php echo $CustInt; ?>&InvNo=<?php echo $InvNo; ?>&TotAmt=<?php echo @$TotAmt; ?>&DT=<?php echo $InvDate; ?>&SDR=<?php echo $SDR; ?>&Summary=<?php echo $Summary; ?>" target=_blank><font size = 3>Click here to add expenses to this invoice (& assign customer to expense)</a></font>
<br>
...
{
    window.location.href = "mailto:<?php echo $CustEmail2; ?>?subject=<?php if ((@$Draft == 'Paid') or (@$Draft == 'Now')) 
		echo "PAID ";  ?>Invoice No <?php echo $InvNo; echo " ".$Summary; ?>&body=<?php echo $body.$nl.$aa.$nl.$a1.$nl.$a2.$nl.$a2b.$nl.$nl.$a3.$nl.$a4.$nll.$a5.$nll.$a6.$nll.$a7.$nll.$nll.$a8.$nll.$a9.$nll.$b1.$nll.$b2.$nll.$b3.$nll.$b5.$nll.$nll.$b6.$nl.$b7.$nl.$nl.$b8.$nl.$b9.$nl.$nl.$ba.$nl.$bb.$nl.$bc.$nl.$nl.$bd.$nl.$bf.$nl.$nl.$bg.$nl.$nl.$bh 
...
tempAlert("<font size = 6> Please attach the PDF file to the email</font>",8000);
...
<table id="data" border="1" cellspacing="0" cellpadding="0" width="75" style="display:none;"></table>
	...
	<dl>
		...
		<dl>
			...
			<input type="submit" name="btn_submit"  value="Create and Display Invoice&#13;&#10;and attach it to the email" style="height:50px; width:170px" /><br>
...
//number_format($UnpaidInvsummm, 2, '.', '');
echo "Total Amount outstanding: &nbsp;&nbsp; <b>R ".number_format($UnpaidInvsummm, 2, '.', '');
...
//echo "<br>".$querySDR;
if ($Tagg ==1) //dont update when the invoice number conflilcts ie when insert fails.
{
...
else 
{
	...
	//echo $queryST;
	echo "<br><font size = 2 color = red><b>Update queryST not successful: <br> ";
